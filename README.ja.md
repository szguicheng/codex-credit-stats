...
`npm start` は、そのターミナルで対話的なフォアグラウンドモードのまま動作します。ダッシュボードの使用中はターミナルを開いたままにしてください。ブラウザを閉じてもローカルサービスは停止しないため、再起動する前に同じターミナルで `Ctrl+C` を押して停止します。ポート競合が表示された場合は、前のインスタンスを実行しているターミナルに戻って `Ctrl+C` を押すか、`npm start -- --port=4318` で別のポートを指定してください。
