...
初回の接続に成功すると、ツールは非機密の接続情報をローカルに保存し、専用のローカルブラウザ profile にあるログイン状態を再利用します。次回以降の起動では、まず自動更新を試みます。保存済みの接続が使えない場合は自動試行を終了し、接続ボタンを再び有効にして、再接続を促します。パスワード、cookie、Authorization token はプロジェクトファイルに書き込みません。
